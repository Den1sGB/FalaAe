...
export default function CriarGrupo({ usuarioAtual, aoVoltar }) {
  ...
  useEffect(() => {
    async function carregarContatosComConversa() {
      // Busca todas as conversas do usuário atual
      const conversasSnap = await database.ref('/conversas').once('value');
      const conversas = conversasSnap.val() || {};
      const idsComConversa = new Set();

      Object.values(conversas).forEach(c => {
        if (
          c.info &&
          Array.isArray(c.info.participantes) &&
          c.info.participantes.length === 2 &&
          c.info.participantes.includes(usuarioAtual.id)
        ) {
          // Pega o outro participante
          const outroId = c.info.participantes.find(pid => pid !== usuarioAtual.id);
          if (outroId) idsComConversa.add(outroId);
        }
      });

      // Busca todos os usuários, mas só mantém quem tem conversa aberta
      const usuariosSnap = await database.ref('/usuarios').once('value');
      const dadosUsuarios = usuariosSnap.val() || {};
      const lista = Object.entries(dadosUsuarios)
        .filter(([id, u]) => id !== usuarioAtual.id && idsComConversa.has(id))
        .map(([id, u]) => ({ id, apelido: u.apelido }));

      setUsuarios(lista);
    }

    carregarContatosComConversa();
  }, [usuarioAtual.id]);
